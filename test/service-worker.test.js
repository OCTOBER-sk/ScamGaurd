...
test("§9.2 openrouter-model-gone: the SPECIFIC §6 rotation message, no generic error", async () => {
  assert.equal(OPENROUTER_DEFAULT_MODEL, "nvidia/nemotron-3-super-120b-a12b:free");
  const fetchImpl = createFetchMock([
    {
      status: 404,
      body: { error: { message: "The model nvidia/nemotron-3-super-120b-a12b:free does not exist or you do not have access to it." } },
    },
  ]);
  const { app, settings } = makeApp({ fetchImpl });
