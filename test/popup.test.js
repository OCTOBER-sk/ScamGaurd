...
test("verdict→seal CSS class mapping matches §1.2 palette tokens", async () => {
  ...
  for (const [verdict, className] of Object.entries(verdictToClass)) {
    ...
    assert.equal(
      ...
      { Safe: "#3F7D5C", Review: "#B5892C", Suspicious: "#C1602B", "High-Risk": "#A3312A" }[verdict],
...
test("§1.2 CSS contains all exact palette tokens", () => {
  ...
  for (const [name, hex] of Object.entries(tokens)) {
    ...
    assert.match(css, pattern, `CSS contains ${name}: ${hex}`);
  }
});
...
test("§1.3 CSS declares @font-face for Fraunces and Inter", () => {
  const css = readFileSync(new URL("../popup.css", import.meta.url), "utf8");

  assert.match(css, /@font-face[\s\S]*?font-family\s*:\s*["']?Fraunces/, "Fraunces @font-face declared");
